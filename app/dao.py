...
#     return db.session.query(func.extract('month', Bill.create_date),
#                             func.sum(TypeRoom.price*RoomForm.count))\
#                         .join(TypeRoom, TypeRoom.rooms == TypeRoom.id)\
#                         .filter(func.extract('year', Bill.create_date) == year)\
#
    #                         .group_by(func.extract('month', Bill.create_date)).all()
